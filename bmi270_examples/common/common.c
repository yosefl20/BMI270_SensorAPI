...
BMI2_INTF_RETURN_TYPE bmi2_i2c_read(uint8_t reg_addr, uint8_t *reg_data,
  ...
  return coines_read_i2c(intf_info.bus, intf_info.dev_addr, reg_addr, reg_data,
                         (uint16_t)len);
}
...
BMI2_INTF_RETURN_TYPE bmi2_i2c_write(uint8_t reg_addr, const uint8_t *reg_data,
  ...
  return coines_write_i2c(intf_info.bus, intf_info.dev_addr, reg_addr,
...
BMI2_INTF_RETURN_TYPE bmi2_spi_read(uint8_t reg_addr, uint8_t *reg_data,
  ...
  return coines_read_spi(intf_info.bus, intf_info.dev_addr, reg_addr, reg_data,
                         (uint16_t)len);
}
...
BMI2_INTF_RETURN_TYPE bmi2_spi_write(uint8_t reg_addr, const uint8_t *reg_data,
  ...
  return coines_write_spi(intf_info.bus, intf_info.dev_addr, reg_addr,
                          (uint8_t *)reg_data, (uint16_t)len);
}
 ...
 */
int16_t coines_board_init(enum coines_comm_intf intf_type,
                          bool get_board_info) {
  ...
  if (result < COINES_SUCCESS) {
    printf("\n Unable to connect with Application Board ! \n"
           ...
           " 3. Check if board is in use by another application. (Insufficient "
           "permissions to access USB) \n");
    exit(result);
  }
...
int8_t bmi2_interface_init(struct bmi2_dev *bmi, uint8_t intf) {
  ...
  if (bmi != NULL) {
    ...
    if (intf == BMI2_I2C_INTF) {
      ...
      /* SDO to Ground */
      coines_set_pin_config(COINES_SHUTTLE_PIN_22, COINES_PIN_DIRECTION_OUT,
    ...
    /* Bus configuration : SPI */
      ...
      result = coines_config_spi_bus(COINES_SPI_BUS_0, COINES_SPI_SPEED_5_MHZ,
                                     COINES_SPI_MODE0);
    ...
    if (COINES_SUCCESS == result) {
      ...
      /* Configure max read/write length (in bytes) ( Supported length depends
       * on target machine) */
...
void bmi2_error_codes_print_result(int8_t rslt) {
  ...
  case BMI2_E_NULL_PTR:
    printf("Error [%d] : Null pointer error. It occurs when the user tries to "
           "assign value (not address) to a pointer,"
  ...
  case BMI2_E_INVALID_SENSOR:
    printf("Error [%d] : Invalid sensor error. It occurs when there is a "
  ...
  case BMI2_E_SELF_TEST_FAIL:
    printf("Error [%d] : Self-test failed error. It occurs when the validation "
           "of accel self-test data is "
           "not satisfied\r\n",
           rslt);
    break;

  case BMI2_E_INVALID_INT_PIN:
    printf("Error [%d] : Invalid interrupt pin error. It occurs when the user "
           "tries to configure interrupt pins "
           "apart from INT1 and INT2\r\n",
           rslt);
    break;

  case BMI2_E_OUT_OF_RANGE:
    printf("Error [%d] : Out of range error. It occurs when the data exceeds "
           "from filtered or unfiltered data from "
           "fifo and also when the range exceeds the maximum range for accel "
           "and gyro while performing FOC\r\n",
           rslt);
    break;

  case BMI2_E_ACC_INVALID_CFG:
    printf("Error [%d] : Invalid Accel configuration error. It occurs when "
           "there is an error in accel configuration"
           " register which could be one among range, BW or filter performance "
           "in reg address 0x40\r\n",
           rslt);
    break;

  case BMI2_E_GYRO_INVALID_CFG:
    printf("Error [%d] : Invalid Gyro configuration error. It occurs when "
  ...
  case BMI2_E_ACC_GYR_INVALID_CFG:
    printf("Error [%d] : Invalid Accel-Gyro configuration error. It occurs "
  ...
  case BMI2_E_CONFIG_LOAD:
    printf("Error [%d] : Configuration load error. It occurs when failure "
  ...
  case BMI2_E_INVALID_PAGE:
    printf("Error [%d] : Invalid page error. It occurs due to failure in "
  ...
  case BMI2_E_SET_APS_FAIL:
    printf("Error [%d] : APS failure error. It occurs due to failure in write "
           "of advance power mode configuration "
           "register\r\n",
           rslt);
    break;

  case BMI2_E_AUX_INVALID_CFG:
    printf("Error [%d] : Invalid AUX configuration error. It occurs when the "
           "auxiliary interface settings are not "
           "enabled properly\r\n",
           rslt);
    break;
  ...
  case BMI2_E_REMAP_ERROR:
    printf("Error [%d] : Remap error. It occurs due to failure in assigning "
  ...
  case BMI2_E_GYR_USER_GAIN_UPD_FAIL:
    printf("Error [%d] : Gyro user gain update fail error. It occurs when the "
           "reading of user gain update status "
           "fails\r\n",
           rslt);
    break;
  ...
  case BMI2_E_INVALID_INPUT:
    printf("Error [%d] : Invalid input error. It occurs when the sensor input "
           "validity fails\r\n",
           rslt);
    break;
  ...
  case BMI2_E_ST_ALREADY_RUNNING:
    printf("Error [%d] : Self-test already running error. It occurs when the "
           "self-test is already running and "
           "another has been initiated\r\n",
           rslt);
    break;

  case BMI2_E_CRT_READY_FOR_DL_FAIL_ABORT:
    printf("Error [%d] : CRT ready for download fail abort error. It occurs "
  ...
  case BMI2_E_DL_ERROR:
    printf("Error [%d] : Download error. It occurs when write length exceeds "
           "that of the maximum burst length\r\n",
           rslt);
    break;

  case BMI2_E_PRECON_ERROR:
    printf("Error [%d] : Pre-conditional error. It occurs when precondition to "
           "start the feature was not "
           "completed\r\n",
           rslt);
    break;

  case BMI2_E_ABORT_ERROR:
    printf("Error [%d] : Abort error. It occurs when the device was shaken "
           "during CRT test\r\n",
           rslt);
    break;

  case BMI2_E_WRITE_CYCLE_ONGOING:
    printf("Error [%d] : Write cycle ongoing error. It occurs when the write "
           "cycle is already running and another "
           "has been initiated\r\n",
           rslt);
    break;

  case BMI2_E_ST_NOT_RUNING:
    printf("Error [%d] : Self-test is not running error. It occurs when "
  ...
  case BMI2_E_DATA_RDY_INT_FAILED:
    printf("Error [%d] : Data ready interrupt error. It occurs when the sample "
           "count exceeds the FOC sample limit "
           "and data ready status is not updated\r\n",
           rslt);
    break;

  case BMI2_E_INVALID_FOC_POSITION:
    printf("Error [%d] : Invalid FOC position error. It occurs when average "
